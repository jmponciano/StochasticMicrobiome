...
ROUSS.pboot <- function(B=2,parms,Yobs, Tvec, REML="FALSE", plot.it="FALSE"){
	...
	if(REML=="TRUE"){
		...
		pred.CIs1 <- cbind(preds.CIs1[,1], reml.preds-(preds.CIs1[,3]-preds.CIs1[,2]),reml.preds,reml.preds+(preds.CIs1[,4]-preds.CIs1[,3]))
...
ROUSS.predict <- function(parms, Yobs,Tvec,plot.it="TRUE"){
	...
	if(plot.it=="TRUE"){	
		...
		X11()
		plot(Time.t,exp(Yobs),xlab="Time",ylab="Population abundance",type="b",cex=1.5, 

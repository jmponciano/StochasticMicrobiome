...
# VERY IMPORTANT:  the order of these labels has to logically match the
# order of the rows in which the cluster means are printed AND the four
# stability classifications as suggested above.  Here, for instance, it is
# the last row that has the largest scaled PC I score and the smallest ddp
# value, so the `highly stable` label goes last.  The cluster mean with the
# ddp value closest to 1 and smallest scaled PC I score appears in the second
# row of the kmeans() output, so the `Highly unstable` label appears second,
# and so forth...
...
A note of caution for the user is necessary:  whenever the kmeans cluster 
algorithm in `R` is run, the order of the clusters is annoyingly printed in a
different order, even for the same data set. So each person running the code 
has to make  sure the color labeling of the points and the belonging labels to 
each cluster all make sense according to the stability classification.  

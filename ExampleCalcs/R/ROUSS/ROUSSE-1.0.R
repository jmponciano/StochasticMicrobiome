...
ROUSS.pboot <- function(B=2,parms,Yobs, Tvec, REML="FALSE", plot.it="FALSE"){
	...
	if(REML=="TRUE"){
		...
		preds.CIs1 <- t(rbind(Tvec,preds.CIs1[1,], reml.preds, preds.CIs1[2,]));
		colnames(preds.CIs1) <- c("Year","2.5%","REMLE","97.5%");

		preds.CIs2 <- apply(preds.boot2,2,FUN=function(x){quantile(x,probs=c(0.025,0.975))});
		preds.CIs2 <- t(rbind(long.t+Tvec[1],preds.CIs2[1,], reml.longpreds, preds.CIs2[2,]));
		colnames(preds.CIs1) <- c("Year","2.5%","REMLE","97.5%");
...
ROUSS.predict <- function(parms, Yobs,Tvec,plot.it="TRUE"){
	...
	if(plot.it=="TRUE"){	
		...
		X11()
		plot(Time.t,exp(Yobs),xlab="Time",ylab="Population abundance",type="b",cex=1.5, main="Predicted (--) and observed (-o-) abundances");#  Population data are circles.
...
ROUSS.CALCS <- function(Yobs,Tvec,pmethod="ML",nboot, plot.pred="TRUE",plot.bootdists = "TRUE"){
	...
	if(pmethod=="ML"){
		...
		out <- list(parms.est = parms.est, lnLhat = lnLhat, AIC = AIC, pbootmat = pboot.calcs[[1]], pboot.cis = pboot.calcs[[2]], pboot.preds = pboot.calcs$preds.CIs)}else{
			out <- list(parms.est = parms.est, lnLhat = lnLhat, pbootmat = pboot.calcs[[1]], pboot.cis = pboot.calcs[[2]], pboot.preds = pboot.calcs$preds.CIs)}
